    ...
    try:
        ...
        if st.button(f"Log ${TRADE_AMOUNT_USD:.2f} Manual Purchase"):
            ...
            if selected_coin_data:
                ...
                new_trade_data = {
                    'timestamp': pd.to_datetime(datetime.now().isoformat()), 'coin_id': selected_coin_data['id'],
                    'name': selected_coin_data['name'], 'symbol': selected_coin_data['symbol'].upper(),
                    'buy_price': analysis_for_trade['current_price'],
                    'quantity': TRADE_AMOUNT_USD / analysis_for_trade['current_price'], 'trade_type': 'manual'
                }
...
if trades_df.empty:
    ...
    # --- ROBUST ID CLEANING (THE FIX) ---
    # 1. Drop any rows that might have a missing coin_id
    cleaned_trades_df = trades_df.dropna(subset=['coin_id'])
    # 2. Get the unique IDs
    unique_ids = cleaned_trades_df['coin_id'].unique()
    # 3. Create a clean list of non-empty strings, stripped of whitespace
    portfolio_coin_ids = [str(id).strip() for id in unique_ids if str(id).strip()]

    # Only proceed if we have valid IDs to query
    if not portfolio_coin_ids:
        ...
        with st.spinner("Fetching latest prices to update portfolio..."):
            try:
                ...
                # Proceed with calculations using the cleaned DataFrame
                ...
                st.error(f"Could not process portfolio data. This might be due to an invalid entry in the trade log or an API issue. Error: {e}")